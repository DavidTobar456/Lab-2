...
PSECT CODE, abs, delta=2 ;Generamos otra sección de código
 ...
 BANKSEL PORTA
    ...
    ; ------LOOP--------------
    loop:
 btfsc PORTB,0 ; Si el push button en ((PORTB) and 07Fh), 0 no da entrada se skipea
 call inc_porta
 btfsc PORTB,1 ; Si el push button en ((PORTB) and 07Fh), 1 no da entrada skipea
 call dec_porta
 btfsc PORTB,2 ; Si el push button en ((PORTB) and 07Fh), 2 no da entrada skipea
 call inc_portc
 btfsc PORTB,3 ; Si el push button en ((PORTB) and 07Fh), 3 no da entrada skipea
 call dec_portc
 btfsc PORTB,4 ; Si el push button en ((PORTB) and 07Fh), 4 no da entrada skipea
 call confirm_suma
    goto loop
 ...
 bsf TRISB,2
 bsf TRISB,3
 bsf TRISB,4 ; Configuramos los de ((PORTB) and 07Fh), 0 a ((PORTB) and 07Fh), 3 como entradas
 ...
 bcf ((OSCCON) and 07Fh), 4 ; Se está usando una frecuencia de 1MHz
 bsf ((OSCCON) and 07Fh), 0 ; Se está usando el oscilador interno
    return

    inc_porta:
 call delay_largo
 btfsc PORTB,0 ; Si el push button en ((PORTB) and 07Fh), 0 no da entrada skipea
 goto $-1
 ...
 bsf PORTA,3
    ...
    inc_portc:
 call delay_largo
 btfsc PORTB,2 ; Si el push button en ((PORTB) and 07Fh), 0 no da entrada skipea
 goto $-1
